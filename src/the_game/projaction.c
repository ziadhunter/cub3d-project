...
/*   Updated: 2025/12/13 21:58:58 by rabounou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_column(t_data *data, int x, int y, int wall_hight)
{
	...
	while (i < tmp)
	{
		ray->y_offset = distance_from_top(i, wall_hight) * ((float)TILE_SIZE
				/ wall_hight);
		put_pixel(&data->new_image, x, i, get_color(ray->hit_cell,
...
void	diplay_btn_msg(t_data *data, int x, int y)
{
	unsigned int	color;
	t_img			*door_btn;
	int				i;
...
void	projaction(t_data *data)
{
	...
	if (data->player->is_looking_at_door == true)
		diplay_btn_msg(data, (WIN_WIDTH / 2) - (120 / 2),
			(WIN_HEIGHT / 4) - (39 / 2));
}
